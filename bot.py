...
async def recurring_category(callback: CallbackQuery, state: FSMContext):
    ...
    days_kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=str(d), callback_data=f"rec_day_{d}") for d in range(26, 32)],
    ])
    cat_label = next((l for l, v in CATEGORIES if v == category), category)
    await callback.message.edit_text(
        f"✅ Категория: <b>{cat_label}</b>\n\n"
        f"Какого числа каждого месяца списывать?\n\n"
        f"⚠️ <i>Если выберешь 29, 30 или 31 — в месяцах где нет такого дня списание пропустится</i>",
        parse_mode="HTML",
